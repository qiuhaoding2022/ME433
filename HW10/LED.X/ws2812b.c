...
#define LOWTIME 5 // number of 48MHz cycles to be low for 0.35uS 15
...
void ws2812b_setColor(wsColor * c, int numLEDs) {
    ...
    char text[100];
    int test = (c[0].r >>3)&1 ;
    sprintf(text,"test is : %d ",test);
    LCD_drawStr(28,40,text,RED);
...
void ws2812b_rainbow(float hu,int numLEDs) {
    int i = 0; int j = 0; // for loops
    int numBits = 2 * 3 * 8 * numLEDs; // the number of high/low bits to store, 2 per color bit
    volatile unsigned int delay_times[2*3*8 * 4]; // I only gave you 5 WS2812B, adjust this if you get more somewhere
    // start at time at 0
    ...
    for (i = 0; i < numLEDs; i++) {
        ...
        sat=0.5+0.1*i;
        bright=0.2+0.05*i;
        wsColor c = HSBtoRGB(hue,sat,bright);
        // loop through each color bit, MSB first
        for (j = 7; j >= 0; j--) {
            // if the bit is a 1
            if ((c.r >>j)&1 ==1)  {
